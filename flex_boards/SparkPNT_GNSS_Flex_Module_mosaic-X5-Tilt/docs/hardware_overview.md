...
-   <a href="https://www.sparkfun.com/sparkpnt-gnss-flex-module-mosaic-x5-im19-imu.html">
	**SparkPNT GNSS Flex Module - mosaic-X5 & IM19 IMU**<br>
	**SKU:** GPS-29457

	---

	<figure markdown>
	![Product Thumbnail](https://cdn.sparkfun.com/assets/parts/3/1/3/6/5/29457-GNSS-Flex-Module-Mosaic-X5-With-IMU-Feature.jpg)
	</figure></a>
...
-   SparkPNT GNSS Flex modules are plug-in boards featuring different GNSS receivers. They are designed to be easily swapped for repairs and pin-compatible for upgrades. The boards have two 2x10-pin, 2mm pitch female headers connecting to carrier boards. For the mosaic-X5 GNSS receiver, these pins will break out the USB, UART (x2), and SD card interfaces, along with the PPS and event signals using a standardized pinout. Additionally, these pins break out the two UART interfaces of the IM19 IMU.
	...
	!!! info "Tilt Compensation"
		...
		When configured, fed with the mosaic-X5 Pulse-Per-Second signal and NMEA GGA, RMC, and GST messages; once calibrated, the IM19 will output proprietary NMEA messages containing the compensated position and roll, pitch and yaw. By default, the mosaic-X5 `COM4` `TX` is linked to the IM19 `UART2` `RX` to carry the required NMEA messages. However, this can be changed via jumper links on the Flex Module, if necessary.
...
-   <!-- Boxes in tabs -->
	...
	=== "Dimensions"
		...
		<figcaption markdown>Dimensions of the mosaic-X5 & IM19 GNSS Flex module.</figcaption>
		</article>
...
The mosaic-X5 & IM19 GNSS Flex module has the following features:
...
<figcaption markdown>Layout of the major components on the mosaic-X5 & IM19 GNSS Flex module.</figcaption>
</figure>
...
When configured, fed with the LG290P Pulse-Per-Second signal and NMEA GGA, RMC, and GST messages; once calibrated, the IM19 will output proprietary NMEA messages containing the compensated position and roll, pitch and yaw. By default, the mosaic-X5 `COM4` `TX` is linked to the IM19 `UART2` `RX` to carry the required NMEA messages. However, this can be changed via jumper links on the Flex Module, if necessary.
...
- 4x UART (LVTTL, up to 4 Mbps) (1)
- ~~Ethernet (RMII/MDIO), 10/100 Mbps~~ (2)
- USB device (2.0, HS)
- SDIO (mass storage)
- ~~2x GPIO user programmable~~ (3)
- 2x LED outputs
- 2x Event markers
...
1. One of the four UART ports is piped to the IM19 module
1. Not available on the mosaic-X5 & IM19 GNSS Flex module.
1. Not available on the mosaic-X5 & IM19 GNSS Flex module.
...
1. The timing signal is amplified from the mosaic-X5 GNSS receiver
...
=== "USB Interface"
	...
	<figcaption markdown>USB interface on the mosaic-X5 & IM19 GNSS Flex module.</figcaption>
	</figure>
...
=== "UART Ports"
	...
	| mosaic-X5    | IM19         | Pins of GNSS Flex Headers |
	| :----------: | :----------: | :------------ |
	| `UART1`      | ----         | `TXD1`/`RXD1` |
	| `UART2`      | ----         | `TXD2`/`RXD2` |
	| ----         | `UART1`      | `TXD3`/`RXD3` |
	...
	<figcaption markdown>The default UART port connections between the mosaic-X5, IM19, and GNSS Flex header pins.</figcaption>
	</figure>
	...
	<figcaption markdown>The UART ports on the mosaic-X5 & IM19 GNSS Flex module.</figcaption>
	</figure>
	...
	**mosaic-X5**
	...
	The mosaic-X5 GNSS receiver has four UARTs ports, which can be operated and configured separately. However, only the `UART1` and `UART2` ports of the mosaic-X5 are available from the headers pins of the GNSS Flex module. The other two UART ports of GNSS Flex system are reserved for the IM19 attitude module.
	...
	<figcaption markdown>The UART ports from the mosaic-X5 on the GNSS Flex module.</figcaption>
	</figure>

	
	- The `UART1` and `UART2` ports of the mosaic-X5 GNSS receiver are broken out to the headers of the GNSS Flex system, on pins `TXD1`/`RXD1`/`RTS1`/`CTS1` and `TXD2`/`RXD2`/`RTS2`/`CTS2` respectively.
		- These can be used to interact with the GNSS receiver.
		- These ports feature flow control pins, which are disabled by default.
		- These ports support a point-to-point protocol server, by which it can accept TCP/IP connections over a serial link.
	- The `TX` pin of the `UART4` port from the mosaic-X5 GNSS receiver is also piped directly to the `RX` pin of the IM19 attitude module's `UART2` port.
	...
	**IM19**
	...
	The IM19 attitude module has two UART ports, which operate separately; and are available from the headers pins of the GNSS Flex module. The other two UART ports of GNSS Flex system are reserved for the mosaic-X5 GNSS receiver. To provide an accurate tilt compensated position, the IM19 attitude module also requires a [PPS signal](#pps-output) that corresponds with the GNSS solutions.
	...
	- The `UART2` port of the IM19 module is used to receive RTK data from the GNSS receiver and output the tilt compensated data.
		- By default, the `RX` pin receives data from the `UART4` port of the mosaic-X5 GNSS receiver.
			- Users can [modify the jumpers](#jumpers) on the top of the GNSS Flex module, to utilize the `TX` pin from the `UART1` or `UART2` ports of the mosaic-X5 GNSS receiver *(i.e. the `TXD1` or `TXD2` pins)*; or the `RXD4` pin of the GNSS Flex headers instead.
...
=== "PPS Output"
	...
	<figcaption markdown>The `PPS` signal output on the mosaic-X5 & IM19 GNSS Flex module.</figcaption>
	</figure>
...
=== "Data Logging"
	...
	<figcaption markdown>The data logging peripherals on the mosaic-X5 & IM19 GNSS Flex module.</figcaption>
	</figure>
...
=== "LED Outputs"
	...
	<figcaption markdown>The LED pins on the mosaic-X5 & IM19 GNSS Flex module.</figcaption>
	</figure>
...
=== "Event Triggers"
	...
	<figcaption markdown>The event pins on the mosaic-X5 & IM19 GNSS Flex module.</figcaption>
	</figure>



## U.FL Connector
Users will need to connect a compatible GNSS antenna to the `L1\L2\L5` U.FL connector. The type of antenna used with the mosaic-X5 GNSS receiver affects the overall accuracy of the positions calculated by the GNSS receiver.
...
<figcaption markdown>The U.FL connector to attach an external GNSS antenna to the mosaic-X5 & IM19 GNSS Flex module.</figcaption>
</figure>
...
<figcaption markdown>The jumpers on the top of the mosaic-X5 & IM19 GNSS Flex module.</figcaption>
</figure>


- `IMU GNSS Source`
:   These four jumpers can be modified to change the source of the GNSS data for the IM19 attitude module.
...
<figcaption markdown>The jumpers on the bottom of the mosaic-X5 & IM19 GNSS Flex module.</figcaption>
</figure>
